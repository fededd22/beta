...
// Internal (127.0.0.1-only, never exposed publicly) xray ports for slot A / B.
// IMPORTANT: hosting platforms inject the public HTTP port as PORT (e.g.
// 10080). If a slot port equals PORT, xray cannot bind (address already in
// use) and our own reverse proxy would loop back into Node itself, so no
// config could ever connect. Shift the slot ports whenever they collide.
function getSlotPorts(slot: string) {
  let base = slot === "B" ? 10090 : 10080;
  while (PORT >= base && PORT <= base + 2) base += 100;
  return { vless: base, vmess: base + 1, trojan: base + 2 };
}
...
function generateV2RayConfigForSlot(slot: "A" | "B", clients: ClientConfig[]) {
  ...
  // No Host header is enforced on the inbounds: the public edge/proxy may
  // rewrite Host, and the domain may not be auto-detected yet when slot A boots.
  // Authentication is done by UUID/password, not by Host.
  ...
  inbounds.push({
    ...
    streamSettings: {
      network: "ws",
      wsSettings: { path: "/by_moon_trojan" },
      sockopt: { tcpFastOpen: true, tcpKeepAliveInterval: 15 }
    }
  });
...
// Override with the SNI_HOST env var (e.g. set it to your real domain if the
// hosting platform's edge does not accept a foreign SNI).
const FAKE_SNI_HOST = process.env.SNI_HOST || "youtube.com";
...
server.on("upgrade", (req, socket, head) => {
  ...
  const url = req.url || "";
  addLog(`[WS] upgrade from ${getRequestIP(req, socket)} host=${req.headers.host || "-"} path=${url.split("?")[0]}`);
